...
def plot_query_retrieval(query_title, neighbor_ids, img_query, imgs_retrieval, outFile=None):
    n_retrieval = len(imgs_retrieval)
    fig = plt.figure(figsize=(2*n_retrieval, 4))
    fig.suptitle("Query: {} | k={}".format(query_title, n_retrieval), fontsize=25)
    ...
    for i, (img, neighbor_id) in enumerate(zip(imgs_retrieval, neighbor_ids)):
        ...
        for axis in ['top', 'bottom', 'left', 'right']:
            ...
            ax.spines[axis].set_color('black')  # set to black
        ax.set_title(neighbor_id)  # set subplot title
...
def plot_tsne(X, imgs, outFile):
    ...
    plt.xlabel('x')
    plt.ylabel('y')
    plt.show()    #plot_embedding(X_tsne, imgs, "t-SNE embeddings")
    if outFile is None:
        plt.show()
    else:
        plt.savefig(outFile, bbox_inches='tight')
